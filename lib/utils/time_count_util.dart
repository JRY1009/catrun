...
class TimeCountUtil {
  ...
  static String SECOND = "秒";
  static String MINUTE = "分";
  static String HOUR = "小时";
...
//        }
  ...
  static String? secondFormat(int second) {
    if (second < 60) {
      int s = second;
      return s.toString() + SECOND;
    } else if (second < 60 * 60) {
      int m = (second / 60).toInt();
      int s = (second % 60).toInt();
      return m.toString() + MINUTE + s.toString() + SECOND;
    } else {
      int h = (second / 3600).toInt();
      int m = ((second % 3600) / 60).toInt();
      int s = (second % 60).toInt();

      return h.toString() + HOUR + m.toString() + MINUTE + s.toString() + SECOND;
    }
  }
}